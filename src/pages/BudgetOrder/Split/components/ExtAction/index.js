...
const { SPLIT_REQUEST_BTN_KEY, REQUEST_VIEW_STATUS } = constants;
const { Item } = Menu;

const menuData = () => [
  {
    title: '查看',
    key: SPLIT_REQUEST_BTN_KEY.VIEW,
    disabled: false,
    ignore: 'true',
  },
  {
    title: formatMessage({ id: 'global.flowHistory', defaultMessage: '流程历史' }),
    key: SPLIT_REQUEST_BTN_KEY.FLOW_HISTORY,
    disabled: true,
    ignore: 'true',
  },
  {
    title: formatMessage({ id: 'global.edit', defaultMessage: '编辑' }),
    key: SPLIT_REQUEST_BTN_KEY.EDIT,
    disabled: true,
    ignore: 'true',
  },
  {
    title: formatMessage({ id: 'global.delete', defaultMessage: '删除' }),
    key: SPLIT_REQUEST_BTN_KEY.DELETE,
    disabled: true,
    ignore: 'true',
  },
  {
    title: '预算确认',
    key: SPLIT_REQUEST_BTN_KEY.CONFIRM,
    disabled: true,
    ignore: 'true',
  },
  {
    title: '撤销确认',
    key: SPLIT_REQUEST_BTN_KEY.CANCEL,
    disabled: true,
    ignore: 'true',
  },
  {
    title: '直接生效',
    key: SPLIT_REQUEST_BTN_KEY.EFFECT,
    disabled: true,
    ignore: 'true',
  },
  {
    title: '启动流程',
    key: SPLIT_REQUEST_BTN_KEY.START_FLOW,
    disabled: true,
    ignore: 'true',
  },
];
...
class ExtAction extends PureComponent {
  ...
  initActionMenus = () => {
    ...
    switch (status) {
      case REQUEST_VIEW_STATUS.DRAFT.key:
        menus.forEach(m => {
          if (
            m.key === SPLIT_REQUEST_BTN_KEY.EDIT ||
            m.key === SPLIT_REQUEST_BTN_KEY.DELETE ||
            m.key === SPLIT_REQUEST_BTN_KEY.CONFIRM
          ) {
      ...
      case REQUEST_VIEW_STATUS.CONFIRMING.key:
        menus.forEach(m => {
          if (m.key === SPLIT_REQUEST_BTN_KEY.CONFIRM || m.key === SPLIT_REQUEST_BTN_KEY.CANCEL) {
            Object.assign(m, { disabled: false });
          }
        });
        break;
      case REQUEST_VIEW_STATUS.CANCELING.key:
        menus.forEach(m => {
          if (m.key === SPLIT_REQUEST_BTN_KEY.CANCEL) {
            Object.assign(m, { disabled: false });
          }
        });
        break;
      case REQUEST_VIEW_STATUS.CONFIRMED.key:
        menus.forEach(m => {
          if (
            m.key === SPLIT_REQUEST_BTN_KEY.START_FLOW ||
            m.key === SPLIT_REQUEST_BTN_KEY.EFFECT ||
            m.key === SPLIT_REQUEST_BTN_KEY.CANCEL
          ) {
            Object.assign(m, { disabled: false });
          }
        });
      ...
      case REQUEST_VIEW_STATUS.EFFECTING.key:
        menus.forEach(m => {
          if (m.key === SPLIT_REQUEST_BTN_KEY.EFFECT) {
            Object.assign(m, { disabled: false });
          }
        });
        break;
      ...
      case REQUEST_VIEW_STATUS.COMPLETED.key:
        menus.forEach(m => {
          if (m.key === SPLIT_REQUEST_BTN_KEY.FLOW_HISTORY) {
            Object.assign(m, { disabled: false });
          }
        });
        break;
  ...
  startFlowCallBack = res => {
    ...
    if (res && res.success && onAction) {
      onAction(SPLIT_REQUEST_BTN_KEY.START_FLOW, recordItem);
    }
  };
  ...
  onActionOperation = e => {
    ...
    if (e.key !== SPLIT_REQUEST_BTN_KEY.START_FLOW) {
      if (manuallyEffective && e.key === SPLIT_REQUEST_BTN_KEY.FLOW_HISTORY) {
        message.destroy();
  ...
  getMenu = (menus, record) => {
    ...
    return (
      ...
      >
        {menus.map(m => {
          if (m.key === SPLIT_REQUEST_BTN_KEY.START_FLOW) {
            return (
