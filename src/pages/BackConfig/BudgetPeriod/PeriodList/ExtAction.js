...
const { getUUID } = utils;
const { BUDGET_PERIOD_USER_ACTION, PERIOD_TYPE } = constants;
const { Item } = Menu;

const menuData = () => [
  {
    title: '编辑',
    key: BUDGET_PERIOD_USER_ACTION.EDIT,
    disabled: true,
  },
  {
    title: '删除',
    key: BUDGET_PERIOD_USER_ACTION.DELETE,
    disabled: true,
  },
  {
    title: '停用',
  ...
  {
    title: '启用',
    key: BUDGET_PERIOD_USER_ACTION.UNFROZEN,
    disabled: false,
  },
];
...
class ExtAction extends PureComponent {
  ...
  initActionMenus = () => {
    ...
    menus.forEach(m => {
      if (m.disabled === false) {
        if (recordItem.closed === true && m.key === BUDGET_PERIOD_USER_ACTION.FROZEN) {
          Object.assign(m, { disabled: true });
        }
        if (recordItem.closed === false && m.key === BUDGET_PERIOD_USER_ACTION.UNFROZEN) {
          Object.assign(m, { disabled: true });
        }
      }
    });
