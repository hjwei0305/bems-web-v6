...
@connect(({ dimensionStrategy, loading }) => ({ dimensionStrategy, loading }))
class DimensionStrategy extends Component {
  static listCardRef = null;

  componentWillUnmount() {
    const { dispatch } = this.props;
    dispatch({
      type: 'dimensionStrategy/updateState',
      payload: {
        currentMaster: null,
        rowData: null,
      },
  ...
  render() {
    const { dimensionStrategy } = this.props;
    const { currentMaster } = dimensionStrategy;
    const masterProps = {
          ...
          <Content className={cls('main-content', 'auto-height')} style={{ paddingLeft: 4 }}>
            {currentMaster ? (
              ...
              <div className="blank-empty">
                <Empty image={empty} description="可选择预算主体配置维度策略" />
              </div>
            )}
          </Content>
        </Layout>
